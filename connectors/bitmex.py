...
class BitmexClient:
    ...
    def _on_message(self, ws, msg: str):
        ...
        if data['table'] == "trade":
            ...
            for d in data['data']:
                ...
                ts = int(dateutil.parser.isoparser(d['timestamp']).timestamp() * 1000)

                for key, strat in self.strategies.items():
                    if strat.contract.symbol == symbol:
                        res = strat.parse_trades(float(data['prices']), float(d['size']), ts)
    ...
    def get_trade_size(self, contract: Contract, price: float, balence_pct: float):
        ...
        xbt_size = balance * balence_pct / 100
